...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
    ...
    if (!brand) return {};
    return { title: brand.seoTitle, description: brand.seoDescription, alternates: { canonical: `https://www.prix-pompe-a-chaleur.be/marques/${brand.slug}` } };
}

function StarRating({ rating }: { rating: number }) {
    return (<div className="flex gap-0.5">{[1,2,3,4,5].map((s) => (<Star key={s} className={`h-5 w-5 ${s <= rating ? "fill-amber-400 text-amber-400" : "fill-slate-200 text-slate-200"}`} />))}</div>);
}
        ...
        <div className="min-h-screen bg-slate-50">
                ...
                <div className="container mx-auto px-4">
                    ...
                    <div className="max-w-3xl mt-6">
                        <div className="flex items-center gap-4 mb-4">
                            ...
                            <div>
                                <h1 className="text-3xl md:text-5xl font-bold">{brand.name} <span className="text-teal-400">Pompe à Chaleur</span></h1>
                                <p className="text-slate-400 mt-1">Depuis {brand.founded} — {brand.headquarters}</p>
                            </div>
                        </div>
                        <p className="text-xl text-teal-300 font-medium">{brand.tagline}</p>
                    </div>
                </div>
            </section>
            <section className="py-12"><div className="container mx-auto px-4"><div className="grid lg:grid-cols-12 gap-10">
                <div className="lg:col-span-8">
                    {brand.heroImage && (
                        <div className="mb-8 rounded-3xl overflow-hidden shadow-xl border border-slate-200">
                            <img src={brand.heroImage} alt={`${brand.name} pompe à chaleur`} className="w-full h-auto object-cover md:h-[400px]" />
                        </div>
                    )}
                    <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-sm mb-8">
                        <h2 className="text-2xl font-bold text-slate-900 mb-4">À propos de {brand.name}</h2>
                        <div className="prose prose-lg prose-slate">{brand.description.split('\n\n').map((p, i) => (<p key={i}>{p}</p>))}</div>
                    </div>
                    <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-sm mb-8">
                        <h2 className="text-2xl font-bold text-slate-900 mb-6">Modèles phares</h2>
                        <div className="grid gap-4">{brand.models.map((m) => (
                            <div key={m.name} className="p-5 bg-slate-50 rounded-xl border border-slate-100">
                                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3">
                                    ...
                                    <div className="flex items-center gap-3 mt-1 sm:mt-0">
                                        <span className="text-xs bg-teal-100 text-teal-700 px-2 py-1 rounded-full font-medium">{typeLabels[m.type]}</span>
                                        <span className="text-sm font-bold text-teal-600">{m.priceRange}</span>
                                    </div>
                                </div>
                                <div className="flex flex-wrap gap-2">{m.features.map((f, i) => (<span key={i} className="text-xs bg-white border border-slate-200 text-slate-600 px-2 py-1 rounded-md">{f}</span>))}</div>
                            </div>
                        ))}</div>
                    </div>
                    <div className="grid md:grid-cols-2 gap-6 mb-8">
                        <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
                            <h3 className="text-lg font-bold text-teal-600 mb-4">✅ Points forts</h3>
                            <ul className="space-y-2">{brand.pros.map((p, i) => (<li key={i} className="flex items-start gap-2 text-sm text-slate-600"><CheckCircle className="h-4 w-4 text-teal-500 shrink-0 mt-0.5" /><span>{p}</span></li>))}</ul>
                        </div>
                        <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
                            <h3 className="text-lg font-bold text-red-500 mb-4">⚠️ Points de vigilance</h3>
                            <ul className="space-y-2">{brand.cons.map((c, i) => (<li key={i} className="flex items-start gap-2 text-sm text-slate-600"><XCircle className="h-4 w-4 text-red-400 shrink-0 mt-0.5" /><span>{c}</span></li>))}</ul>
                        </div>
                    </div>
                    <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-sm">
                        ...
                        <Accordion type="single" collapsible className="w-full">{brand.faq.map((f, i) => (
                ...
                <div className="lg:col-span-4 space-y-6">
                    <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm sticky top-24">
                        ...
                        <Link href="/devis"><Button className="w-full bg-teal-600 hover:bg-teal-700 text-white rounded-xl h-12"><Shield className="mr-2 h-4 w-4" />Devis Gratuit</Button></Link>
                    </div>
                    <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
                        <h3 className="text-lg font-bold text-slate-900 mb-4">Autres marques</h3>
                        <div className="space-y-3">{otherBrands.map((o) => (
                            <Link key={o.slug} href={`/marques/${o.slug}`} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg hover:bg-teal-50 transition-colors group">
                                <div className="flex items-center gap-2"><span>{o.countryFlag}</span><span className="font-medium text-slate-700 group-hover:text-teal-600">{o.name}</span></div>
            ...
            <section className="py-16 bg-teal-900 text-white">
                <div className="container mx-auto px-4 text-center">
                    ...
                    <p className="text-xl text-teal-100 mb-8 max-w-2xl mx-auto font-light">Comparez les devis d&apos;installateurs certifiés RESCert en Belgique.</p>
                    <Link href="/devis"><Button size="lg" className="bg-teal-500 hover:bg-teal-400 text-white text-lg px-10 h-14 rounded-full shadow-lg border border-teal-400">Obtenir un devis<ArrowRight className="ml-2 h-5 w-5" /></Button></Link>
                </div>
            </section>
        </div>
    );
