            ...
            <div className="container mx-auto px-4">
                ...
                <nav aria-label="Villes à proximité" className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {nearby.map((c) => (
                        <Link
                            key={c.slug}
                            href={`/prix-pac/${c.slug}`}
                            className="text-slate-600 hover:text-teal-600 hover:bg-teal-50 p-2 rounded-md transition-colors text-sm truncate block"
                            title={`Prix pompe à chaleur à ${c.name} (${c.zip})`}
                        >
